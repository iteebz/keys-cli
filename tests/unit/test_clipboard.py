...
def test_get_clipboard_macos(mock_check_output, mock_platform_system):
...
def test_set_clipboard_macos(mock_run, mock_platform_system):
    mock_platform_system.return_value = "Darwin"
...
@patch("subprocess.check_output")
def test_get_clipboard_linux(mock_check_output, mock_platform_system):
    mock_platform_system.return_value = "Linux"
    mock_check_output.return_value = "clipboard content from linux"

    result = get_clipboard()

    mock_check_output.assert_called_once_with(["xclip", "-o", "-selection", "clipboard"], text=True)
...
def test_get_clipboard_linux_empty_on_error(mock_check_output, mock_platform_system):
    mock_platform_system.return_value = "Linux"
...
@patch("subprocess.run")
def test_set_clipboard_linux(mock_run, mock_platform_system):
    mock_platform_system.return_value = "Linux"
...
@patch("subprocess.run")
def test_paste_macos(mock_run, mock_platform_system):
    mock_platform_system.return_value = "Darwin"
...
@patch("subprocess.run")
def test_paste_linux(mock_run, mock_platform_system):
    mock_platform_system.return_value = "Linux"
...
def test_get_clipboard_unsupported_platform(mock_platform_system):
    mock_platform_system.return_value = "Windows"
    with pytest.raises(NotImplementedError, match="Platform not supported: Windows"):
        get_clipboard()


def test_set_clipboard_unsupported_platform(mock_platform_system):
    mock_platform_system.return_value = "Windows"
    with pytest.raises(NotImplementedError, match="Platform not supported: Windows"):
        set_clipboard("some text")


def test_paste_unsupported_platform(mock_platform_system):
    mock_platform_system.return_value = "Windows"
    with pytest.raises(NotImplementedError, match="Platform not supported: Windows"):
        paste()
